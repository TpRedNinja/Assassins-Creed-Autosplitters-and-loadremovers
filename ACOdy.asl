...
start
{
    ...
    {
        if (vars.getWorld(current.world) == "ACD_Greece" && old.inIntroCutscene == 1 && current.inIntroCutscene == 0 && current.isLoading == 0)
        {
            vars.introDone = true;
            return true;
        }
    ...
    {
        if (vars.getWorld(current.world) == "ACD_Greece" && current.inPrologueCutscene == 0 && current.isLoading == 0)
            return true;
    }
}
...
split
{
    if (current.lastCompletedQuest == 0 || current.lastCompletedQuest == old.lastCompletedQuest)
            return false;
